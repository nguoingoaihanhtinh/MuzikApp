...
**Tests**: The examples below include test tasks. Tests are OPTIONAL - only include them if explicitly requested in the feature specification. However, per Constitution, behavior changes MUST include corresponding tests (backend unit and/or frontend component/integration tests), and CI will enforce basic coverage gates.
...
- Paths shown below assume this repository's structure. Adjust if plan.md defines feature-specific folders.
...
**NOTE: Write these tests FIRST, ensure they FAIL before implementation. CI requires tests for behavior changes.**
